...
class ZipDatasetPackager:
    def package(self, exported_file: Path, manifest: GenerationManifest, destination: Path, validation_report: dict | None = None, quality_review: dict | None = None, dataset_spec: dict | None = None) -> Path:
        ...
        with ZipFile(destination, "w", ZIP_DEFLATED) as archive:
            ...
            metadata = {"datasetName": "Dataset Forge dataset", "sourceFile": manifest.source_file, "recordCount": manifest.record_count, "format": manifest.requested_format.value, "generatedAt": manifest.generated_at.isoformat(), "schemaVersion": manifest.schema_version, "generationMode": manifest.phase, "provider": manifest.provider, "model": manifest.model, "promptVersion": manifest.prompt_version, "validationStatus": report.get("status", manifest.validation_status), "groundingStatus": (report.get("grounding") or {}).get("status", "not_evaluated"), "groundedRecords": (report.get("grounding") or {}).get("grounded_records", 0), "finalRecordCount": manifest.accepted_record_count or quality.get("final_record_count", quality.get("acceptedRecords", manifest.record_count)), "generatedCandidateCount": manifest.generated_candidate_count or manifest.record_count, "acceptedRecordCount": manifest.accepted_record_count or manifest.record_count, "exportedRecordCount": manifest.exported_record_count or manifest.record_count, "recordCountMode": manifest.record_count_mode, "requestedRecordCount": manifest.requested_record_count, "recommendedRecordCount": manifest.recommended_record_count, "estimatedRecordCountMin": manifest.estimated_record_count_min, "estimatedRecordCountMax": manifest.estimated_record_count_max, "autoStopReason": manifest.auto_stop_reason, "exactDuplicatesRemoved": quality.get("exact_duplicates_removed", quality.get("duplicateRecords", 0)), "nearDuplicateCount": quality.get("near_duplicate_pairs", 0), "qualityReviewVersion": (quality_review or {}).get("reviewVersion"), "reviewProvider": (quality_review or {}).get("provider"), "reviewModel": (quality_review or {}).get("model"), "reviewPromptVersion": (quality_review or {}).get("promptVersion"), "reviewStatus": (quality_review or {}).get("status", "not_evaluated"), "reviewRecommendation": (quality_review or {}).get("recommendation"), "reviewFindingVerification": (quality_review or {}).get("reviewFindingVerification"), "revisionAttempted": (quality_review or {}).get("revisionAttempted", False), "revisionSucceeded": (quality_review or {}).get("revisionSucceeded", False), "revisionSkipReason": (quality_review or {}).get("revisionSkipReason"), "revisionCount": 1 if (quality_review or {}).get("revisionAttempted") else 0}
