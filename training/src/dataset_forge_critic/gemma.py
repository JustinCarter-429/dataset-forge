...
from .e2_contract import canonical_json, parse_strict_response, target_from_record
...
def render(record:dict[str,Any],layer:str)->dict[str,Any]:
 ...
 return {'record_id':r['record_id'],'source_layer':layer,'renderer_version':RENDERER,'system_prompt':(Path(__file__).resolve().parents[2]/'prompts'/'critic-system-v2.txt').read_text(encoding='utf-8'),'user_payload':stable_json(payload),'completion':completion,'requested_targets':requested}
